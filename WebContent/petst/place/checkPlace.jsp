...
<body>
	...
	<div class="container" style="width: 1300px; border: 1px solid 000000; border-radius: 5px;">
		<div id="searchlongBar">
			<div class="option" align="center"
				style="width: 1100px; height: 180px; margin: 0 auto; background:#bebebe; border-radius: 5px;">
				<div style="background:#323232; border-radius: 5px;">		
				<br>
<!-- 				<font style="font-size: 25px; color: white; align-content: left;" >키워드 선택</font>&nbsp;&nbsp;&nbsp; -->
					<ul style="text-align: center;">
					
						<!-- 		style ="background:#FFEE58;" -->
						<li class="CCChoice" onclick="" style="font-size: 25px; color: white; align-content: left;" >키워드 선택</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 동물병원')" id="p0">동물병원</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 애견카페')" id="p1">애견카페</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 고양이카페')" id="p2">애묘카페</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 반려동물용품')" id="p3">반려동물용품</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 반려동물 미용실')" id="p4">미용실</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 반려동물 호텔')" id="p5">호텔</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 공원')" id="p6">공원</li>
						<li class="placeCate"
							onclick="changeColor(this); selectPlaces(' 반려견 놀이터')" id="p7">놀이터</li>
					</ul>
					<br>
				</div>
				<br>
				<div style="height: 4px;">
				<font style="font-size: 25px;color: #323232; align-content: left;" >지역검색</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<input type="text"  value="${key }" id="keyword" size="20"  
						 style="height: 40px;  width: 700px; cursor: text; text-align: center; border: 1px solid #B4E5FF;font-size: 25px;font-weight: bold; ">&nbsp;&nbsp;&nbsp;
					<div id="submitBtn">
						<i class="fa fa-search"style="color : #B4E5FF;" onclick="searchPlaces()"></i>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="wrapper" style="width: 1300px;height: 100%;border-radius: 5px;">
		
			<div class="form-group" style="height: 100%;">
				<div class="col-md-9" style="height: 100%;">
					<div id="all" style="width: 100%; height: 100%;border: 1px solid #b4b4b4; background-color:#323232; border-radius: 5px;">
						<!-- 매우중요함 all 이라느 id를 가진 디브가 지도랑 구분을 지어줌 -->
						<div class="map_wrap">
							<div id="map"
				...
				<div class="col-md-3" style="height: 100%;">
					<div id="hospitalReview" style="width: 100%; height: 100%; border: 1px solid #323232; border-radius: 5px;">
						<div class="hospitalName" align="center" style="background-color: #323232; padding: 10px;">
											
							병원명 <i class="fa fa-hospital"></i>
						</div>
						<input type="hidden" class="hspitalName"><input	type="hidden" id="hospitalX"> 
						<input type="hidden" id="hospitalY">
						<div id="reviewList"></div>
						<div id="writeReview" style="background-color: #323232;">
							<div style="width: 100%; padding: 0px;">
								<textarea id="writeText" rows="5"
									style="width: 100%; padding: 10px;border: 1px solid #bebebe;">후기를 작성해주세요.</textarea>
	...
	<script>
		...
		function changeColor(obj) {
			...
			for (var i = 0; i < placeCate.length; i++) {
				placeCate.item(i).style.background = "#323232";
			}
			obj.style.background = '#0A82FF';
		}
		// 		if(${mainPlace!= null}){
		...
		function getListItem(index, places) {
			...
			itemStr += '<br><a href=' + places.place_url + ' target="_blank" style="padding: 8px; background-color: #28A0FF; color: white; font-weight: bold; border: 1;  border-radius: 5px;">상세보기</a>';
			var placesValue = places.x + '/' + places.y + '/' + places.place_name;
			if (index == 0) {
				itemStr += '&nbsp;&nbsp;<button id="placeReview1" value=' + placesValue + ' style=" padding: 5px; background-color: #50C8FF; color: white; font-weight: bold; border: 1;  border-radius: 5px;">펫프 리뷰보기</button>' +
					'</div>';
			} else {
				itemStr += '&nbsp;&nbsp;<button class="placeReview" value=' + placesValue + ' style="padding: 5px; background-color: #50C8FF; color: white; font-weight: bold; border: 1; border-radius: 5px;">펫프 리뷰보기</button>' +
					'</div>';
			}
				...
				$.ajax({
					...
					success : function(data) {
						...
						for (var i in data) {
							...
							placeStr += '<div class="reviewBoard style="padding:10px;"><div><div>'
							+'<i class="fa fa-paw" style="font-size : 25px; color : #B4E5FF;"></i>&nbsp; <span class="p_nick">'
							+data[i].place_usernickname+'</span></div><div style="display:inline-block; width: 10%;">'
							+'</div><div class="place_onereview" style="display:inline-block; width: 85%;">'
							+ place_review +'<br></div><br>';
							if(sessionId==data[i].place_userid || ${sessionScope.adminCheck !=null}){
				...
				if (place_review == '후기를 작성해주세요.' || place_review.length < 10)
					...
					$.ajax({
						...
						success : function(data) {
							...
							+data.place_usernickname+' '+'</span></div><div style="display:inline-block; width: 10%;"></div>'
				...
				if (place_review == '후기를 작성해주세요.' || place_review.length < 10)
					...
					$.ajax({
						...
						success : function(data) { 
							...
							var placeStr = '';
							placeStr += '<div class="reviewBoard"><div><div>'
							placeStr += '<i class="fa fa-paw" style="font-size : 25px; color : #B4E5FF;"></i>&nbsp;<span class="p_nick"'
							placeStr += 'style="font-size: 18px; font-weight : bold;">'
							placeStr += data.place_usernickname+' '+'</span></div><div style="display:inline-block; width: 10%;"></div>'
...
// 			var mainplace1 = '#'+'${mainPlace1}';
			$('#'+'${mainPlace1}').css('background-color','#0A82FF');
