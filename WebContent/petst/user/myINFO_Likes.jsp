...
<c:forEach items="${myLikesList}" var="like">
		...
		<input type="hidden" value = "${like.boardno}"  >	
					...
					<td align="center"  style ="font-weight:bold;">${like.userId }</td>
...
<%-- 					<fmt:formatDate value="${like.writeDate }"pattern="yyyy-MM-dd" /> --%>
				...
				<ul class="pagination">
					...
					<c:if test="${start != 1 }">
						<li><a href="myWritesList.do?page=1">[처음]</a></li>
						<li><a href="myWritesList.do?page=${start-1 }">[이전]</a></li>
					</c:if>
					...
					<c:if test="${end != last }">
						<li><a href="myLikesList.do?page=${end+1 }">[다음]</a></li>
						<li><a href="myLikesList.do?page=${last }">[끝]</a></li>
					</c:if>
